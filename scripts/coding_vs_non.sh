...
cat ~/urchin_af/variants/urchin_final.vcf | grep -v '^#' | awk '{OFS="\t"; {print "chr"$1,$2-1,$2}}' |sort -k1,1 -k2,2n | sed 's/^chr//g'| ~/bin/bedtools2/bin/bedtools closest -a stdin -b ~/reference/urchin_probes.sorted.nospace.bed \
...
echo "number of significant variants in exons"
...
echo "number of exon probes represented for sig variants"
cat ~/urchin_af/analysis/cmh.selected.bed| sort -k1,1 -k2,2n | sed 's/^chr//g'|  ~/bin/bedtools2/bin/bedtools closest -a stdin -b ~/reference/urchin_probes.sorted.nospace.bed \
    -d -t first | awk ' $9=="exon" {print $0}' | cut -f 7 | sort | uniq | wc -l
