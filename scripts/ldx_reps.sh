...
## this version separates all samples. Previous version lumped D1, and each D7 samples
...
#$samtools view -H /data/OASV2/bwamem/fullymapped/bwamem_final.merged.sorted.bam | grep "^@RG" | cut -f 2 | cut -f 2 -d ":" | cut -f 1 -d "-" | sort | uniq

#check for presence of D1 bams
for in_bam in `$samtools view -H /data/OASV2/bwamem/fullymapped/bwamem_final.merged.sorted.bam | grep "^@RG" | cut -f 2 | cut -f 2 -d ":" | cut -f 1 -d "-" | sort | uniq`

do

    $samtools view /data/OASV2/bwamem/fullymapped/bwamem_final.merged.sorted.bam  | grep $in_bam | cat ~/urchin_af/variants/sam.header - | $samtools view -Sb > ~/urchin_af/variants/${in_bam}.bam
    $samtools index ~/urchin_af/variants/${in_bam}.bam
